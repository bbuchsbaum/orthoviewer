...
This plan coordinates work across three codebases: - `orthoviewer` (R
package, current repo) – HTML widgets, Shiny integration, R-facing
API. - `~/code/jscode/neuroimjs` – JavaScript core viewer + controls
(orthogonal viewer, layer panel). - `~/code/neuroim2` – R-side neuroim
data structures and volume serialization.

Progress is tracked in `Progress.md`; architecture is summarized in
`Architecture.md`.
...
- [`ortho_viewer()`](reference/ortho_viewer.md) to initialize with a
  background volume.
- [`ortho_viewerOutput()`](reference/ortho_viewerOutput.md) and
  [`renderOrtho_viewer()`](reference/renderOrtho_viewer.md) for Shiny.

Implement R→JS proxy API in `orthoviewer`:

- [`ortho_proxy()`](reference/ortho_proxy.md) R6 class with methods:
  - `add_layer()`, `set_window()`, `set_threshold()`, `set_colormap()`,
    `set_opacity()`.
...
- Convert R volume objects (from `neuroim2`) to JSON specs for
  `neuroimjs`.
...
**\[neuroimjs\]** Ensure orthogonal viewer factory supports
`"left-tall"` layout:
...
- Panel introspects available layers and colormaps and updates them via
  existing handlers.
...
Apply warm paper + ochre/teal theme to `ortho_viewer` shell in
`orthoviewer`:
...
- Replace `static styles = css\`…\`\` with Albers-inspired block (warm
  gradient, system UI font, scoped Shoelace tokens).
- Ensure no behavior changes—style only.

Optionally add threshold semantics caption in `layer-control-panel`
template:

- “Values \< low or \> high are opaque; between low and high are
  transparent.”
...
- Check that layers align across all three views (axial, coronal,
  sagittal).
...
- Adjust colors, spacing, and typography to keep the composition clean
  and minimal.
- Optionally add viewer chrome (pane labels, subtle frames) consistent
  with the sidebar.
